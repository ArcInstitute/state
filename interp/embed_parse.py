#!/usr/bin/env python
"""
embedding_visualization.py – v6
===============================
Visualizes embedding differences across ALL cell types in the dataset.
Focuses on comprehensive embedding analysis across multiple cell types.

The script now:
* Loads cell data and samples cells from ALL available cell types
* Extracts and compares embeddings between all cell types
* Creates various visualizations to show embedding relationships
* Performs statistical analysis of embedding distributions across cell types
"""
...
# List of donor file paths
DONOR_FILES = [
    "/large_storage/ctc/ML/state_sets/parse_chunked/Donor1_emb.h5ad",
    "/large_storage/ctc/ML/state_sets/parse_chunked/Donor2_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor3_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor4_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor5_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor6_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor7_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor8_emb.h5ad",
    # "/large_storage/ctc/ML/state_sets/parse_chunked/Donor9_emb.h5ad",
]

FIG_DIR = Path(__file__).resolve().parent / "figures" / "embedding_parse" 

# Load and concatenate all donor files, adding a donor column
adatas = []
for donor_path in DONOR_FILES:
    donor_name = Path(donor_path).stem.split('_')[0]  # e.g., 'Donor1'
    ad = sc.read_h5ad(donor_path)
    ad.obs = ad.obs.copy()  # Ensure obs is writeable
    ad.obs['donor'] = donor_name
    adatas.append(ad)

adata_full = sc.concat(adatas, join='outer', label=None, index_unique=None)
logger.info("Loaded and concatenated all donor files. Combined shape: %s", adata_full.shape)
...
control_pert = "PBS"  # Default control
if "cytokine" in adata_full.obs.columns:
    drugname_counts = adata_full.obs["cytokine"].value_counts()
    # Look for common control names
    for potential_control in ["PBS", "non-targeting", "control", "DMSO"]:
...
for cell_type in valid_cell_types:
    ...
    cells_this_type = adata_full[(adata_full.obs["cytokine"] == control_pert) & 
